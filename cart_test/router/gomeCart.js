...
function miniCart(req,res){
	res.render("miniCart"+(req.query.debug? '-debug' : ''))
}
//公共头 本地测试用
...
//苹果特惠--学生提交个人信息页
function appleTeHuiProfile(req,res){
    res.render("appletehui-profile" + (req.query.debug? '-debug' : ''),{
        config:req.cartconfig
    });
}
router.get("/",cart);
...
router.get("/minicart",miniCart);
router.get('/hygTip',hygTip);
router.get('/appletehui-profile',appleTeHuiProfile);

;(function (exports) {
    ...
    var cartList = '<div class="mini-products">\
                        ...
                        <ul class="mini-nProductLists">\
                            <% for(var i=0;i<miniCommerceItemsGroupVOs.length;i++ ) { %>\
                            <li class="mini-list clearfix">\
                                <% var miniVOs = miniCommerceItemsGroupVOs[i] || {}; %>\
                                <dl class="nProduct-huod realMini">\
                                    <% if(miniVOs.promotionHeads && miniVOs.promotionHeads.length > 0 ) { %>\
                                    <% if(!(miniVOs.promotionHeads[0] && miniVOs.promotionHeads[0].type=="TZ")) {%>\
                                    <dd data-sub="tit-sub" class="huod-hd clearfix">\
                                        <% for(var k=0; k< miniVOs.promotionHeads.length; k++) {%>\
                                        <% var promotionHeadItem = miniVOs.promotionHeads[k] || {},label = promotionHeadItem.label ;%>\
                                        <div class="pCol-name-huod">\
                                            <% if (promotionHeadItem.type === "MJ") {%>\
                                                <% var promotionTxt = ""; %>\
                                                <% if(promotionHeadItem.satisfied) {%>\
                                                    <span class="ico-l">满减</span>\
                                                    <% promotionTxt = "活动商品已购满" + ($toFixed(promotionHeadItem.preC)) + "元（已减" + ($toFixed(promotionHeadItem.postC)) + "元）";%>\
                                                <% } else if (promotionHeadItem.validKindNum) {%>\
                                                     <span class="ico-l-gray"><i></i>满减</span>\
                                                     <% promotionTxt = "活动商品购满" + ($toFixed(promotionHeadItem.preC)) + "元不同种类商品，即可享受满减促销活动";%>\
                                                <% } else {%>\
                                                    <span class="ico-l-gray"><i></i>满减</span>\
                                                    <% promotionTxt = "活动商品购满" + ($toFixed(promotionHeadItem.preC)) + "元，即可享受满减";%>\
                                                <% } %>\
                                                <span class="nLowLeight " title="<%= promotionTxt %>"><%=$cutstr(promotionTxt,26)  %></span>\
                                            <% } else if (promotionHeadItem.type === "ZDZ"|| promotionHeadItem.type === "ZD") {%>\
                                                <% var promotionTxt = ""; %>\
                                                <% if(promotionHeadItem.satisfied) {%>\
                                                    <span class="ico-l">满折</span>\
                                                    <% promotionTxt = "活动商品已购满" + (promotionHeadItem.preC || "0") + "件（已减" + ($toFixed(promotionHeadItem.postC)) + "元）";%>\
                                                 <% } else if (promotionHeadItem.validKindNum) {%>\
                                                    <span class="ico-l">满折</span>\
                                                    <% promotionTxt = "购买" + (promotionHeadItem.preC || "0") + "件不同种类商品，即可享受满折活动";%>\
                                                 <% } else {%>\
                                                    <span class="ico-l-gray"><i></i>满折</span>\
                                                    <% promotionTxt = "活动商品购满" + (promotionHeadItem.preC || "0") + "件，即可享受满折";%>\
                                                <% } %>\
                                                <span class="nLowLeight " title="<%= promotionTxt %>"><%=$cutstr(promotionTxt,26)  %></span>\
                                            <% } else {%>\
                                                <span><%=$showTypeTitle(promotionHeadItem) %></span>\
                                            <% } %>\
                                        </div>\
                                        <%} %>\
                                        <% if(miniVOs.miniGroupSummaryVO && miniVOs.miniGroupSummaryVO.promtion && miniVOs.miniGroupSummaryVO.subAmount > 0) {%>\
                                        <p style="padding-left:8px;">\
                                            小计：<span class="yuan" style="margin-right:10px;"> <b>¥</b><%= $toFixed(miniVOs.miniGroupSummaryVO.subAmount) %></span>\
                                            <% if(miniVOs.miniGroupSummaryVO.promoDiscAmount > 0) {%>\
                                            <span class="nRed">已减：<%= $toFixed(miniVOs.miniGroupSummaryVO.promoDiscAmount)%>元</span>\
                                            <% } %>\
                                        </p>\
                                        <%} %>\
                                    </dd>\
                                    <%} %>\
                                    <%} %>\
                                    <% var commerceItemsGroup = miniVOs.commerceItemsGroup || [];%>\
                                    <% if(miniVOs.promotionHeads && miniVOs.promotionHeads.length > 0 && miniVOs.promotionHeads[0].type=="TZ"){ %>\
                                        ...
                                        <dd class="mini-huod-list min-sideline-list clearfix">\
                                            <div class="mini-product-info clearfix">\
                                                ...
                                                <div class="mini-pCol-name">\
                                                    <p>\
                                                        <a target="_blank" class="nMinCartBlue" title="<%= commerceItemsGroup[0].vProductName %>" href="<%= commerceItemsGroup[0].vUrl %>">\
                                                            【套装】<%=commerceItemsGroup[0].vProductName  %></a>\
                                                    </p>\
                                                </div>\
                                                <div class="mini-pCol-row">\
                                                    <% if(miniVOs.miniGroupSummaryVO && miniVOs.miniGroupSummaryVO.subAmount){ %>\
                                                        <span class="yuan"><b>¥<%= $toFixed(miniVOs.miniGroupSummaryVO.subAmount) %></b></span>\
                                                    <% }%>\
                                    ...
                                    <%}else{%>\
                                        ...
                                        <dd class="mini-huod-list min-sideline-list clearfix">\
                                            <div class="mini-product-info clearfix">\
                                                <div class="mini-pCol-img">\
                                                    <a  style="position: relative;" target="_blank" title="<%= commerceItem.itemName %>" href="<%= commerceItem.itemURL %>">\
                                                       <%=$showTuanProductTip(commerceItem) %>\
                                                       <img alt="" src="<%= commerceItem.itemImageURL %>" width="50" height="50"></a>\
                                                </div>\
                                                <div class="mini-pCol-name">\
                                                    <p>\
                                                        <a target="_blank" class="nMinCartBlue" title="<%= commerceItem.itemName %>" href="<%= commerceItem.itemURL %>"><%=$cutstr(commerceItem.itemName,56)  %></a>\
                                                    </p>\
                                                </div>\
                                                <div class="mini-pCol-row">\
                                                    ...
                                                    <% if(commerceItemsGroup[j].commerceitemVOFlag != "SUB"){ %>\
                                                        <a class="nLowLeight miniDel" href="javascript:void(0)" title="删除" data-itemid="<%= commerceItem.itemId %>">删除</a>\
                                                    <% }%>\
                                                </div>\
                                            </div>\
                                            <% var incrementServiceItems = commerceItem.miniIncrementServiceInfoVOs || [], commerceItemVOs = commerceItem.showBaseGroupVOList || [];%>\
                                            ...
                                            <div class="mini-pCol-promotion fl">\
                                                <%for(var k=0;k<incrementServiceItems.length;k++) {%>\
                                                    <% var dName = (incrementServiceItems[k].promotionType == "1"  ? "特惠" : "") + $toDisplayName(incrementServiceItems[k]) + "  " + incrementServiceItems[k].numOfYear + "年  ¥" + $toFixed(incrementServiceItems[k].price) %>\
                        ...
                        <div id="atg_store_csFooter1" class="cartfooter">\
                            <h4>\
                                已选 <i><span id="csQuantity"><%= miniCartProfileVO.itemCount || 0 %></span></i> \
                                件 ，共计： <strong><span id="csSubtotal">¥</span><%= $toFixed(miniCartProfileVO.totalAmount) %></strong>\
                            </h4>\
                            <% if(isUpCart) {%>\
                                <a  data-code="cart01002" id="csCheckout" style="text-decoration:none;" class="gocart" href="//cart' + cookieDomain  + site+ '/cart" title="去购物车">\
                                    <span>去购物车</span>\
                                </a>\
                            <% } else {%>\
                                <a  data-code="cart01001" id="csCheckout" style="text-decoration:none;" class="gocart" href="//cart' + cookieDomain  + site + '/cart" title="去购物车">\
                                    <span>去购物车 </span>\
                                </a>\
                            <% } %>\
                        </div>\
    ...
    var emptyCart = '<div carttype="asidecart"  id="csEmptyMessage" class="noshop">\
                        <% if(isUpCart) {%>\
                            <a data-code="cart01002" href="//cart' + cookieDomain + site + '/cart" title="去购物车" style="text-decoration:underline;">购物车</a>\
                        <% } else {%>\
                            <a data-code="cart01001" href="//cart' + cookieDomain + site + '/cart" title="去购物车" style="text-decoration:underline;">购物车</a>\
                        <% } %>\
                        中还没有商品，赶紧选购吧！</div>';
    ...
    var URL = (function(){
        ...
        return {
            loadCart: "//cart" + cookieDomain + site+"/api/cart/loadMiniCart"
            , delCartItem: "//cart" + cookieDomain + site+"/api/cart/removeItem"
            , changeNum: "//cart" + cookieDomain + site+"/api/cart/changeNum"
        }
    })();
    ...
    function tplHelp() {
        ...
        template.helper('$cutstr', cutstr);
        template.helper('$toFixed', function (arg) {
            return parseFloat(arg || 0).toFixed(2);
        });
        template.helper('$showTuanProductTip', function (good) {
            var html = '<span style="position: absolute;display:inline-block;height:17px;width:17px;background-color:#f95b5a;color:#fff;font-size:12px;line-height:17px;overflow:hidden;text-align:center;font-weight:800;">';
            if(!good || !good.belonging){
                return '';
            }
            if(good.inventoryState== 'NO_GOODS' || good.inventoryState== 'OFF'){
                return ''
            }
            if(good.belonging === 'QIANG'){
                html +='抢'
            }else if(good.belonging === 'TUAN'){
                html += '团'
            }
            html += '</span>';
            return html;
        ...
        template.helper('$toServiceType', function (item) {
            ...
            if (!item || (typeof item.serviceType ==='undefined')) {
                return '';
            }
        ...
        template.helper('$isTypeTitle', function (type) {
            return TYPES[type] ? true : false;
        });
        template.helper('$showTypeTitle', function (obj) {
            var type = obj.type
                ,value = TYPES[type] || '促销'
                ,isSolid = obj.satisfied
                ,html = '';
            if(isSolid){
                html += '<span class="ico-l">'+value+'</span>';
            }else{
                html +='<span class="ico-l-gray"><i></i>'+value+'</span>';
            }
            if(type === 'DPG'){
                obj.label = '搭配购优惠组合';
            }
            html+=' <span class="nLowLeight " title="' + obj.label +'">'+cutstr(obj.label,26)+'</span>';
            return html;
        });
    };
     ...
     * 处理data数据
     * 层层 就像洋葱一样一层剥一层
     *    最终在加属性commerceItemsGroup._serverList-延保碎屏保在当前层处理
     * @param data
     */
    ...
    //         return ;
    //     }
    //     var groups = data.miniCommerceItemsGroupVOs;
    //     if(groups && groups.length>0){
    //         for(var i=0,len=groups.length; i<len; i++){
    //             var group = groups[i];
    //             var com = group.commerceItemsGroup;
    //             if(com && com.length>0 ){
    //                 for(var j=0,l=com.length; j<l; j++){
    //                     var _serverList = [];
    //                     if(!com[j] || !com[j].miniIncrementServiceInfoVOs
    //                         || !com[j].miniIncrementServiceInfoVOs.showBaseGroupVOList){
    //                         continue;
    //                     }
    //                     var info = com[j].miniIncrementServiceInfoVOs.showBaseGroupVOList;
    //                     for(var y=0,yl=info.length; y<yl; y++ ){
    //                         if(info[y].miniIncrementServiceInfoVOs
    //                             && info[y].miniIncrementServiceInfoVOs.length>0){
    //                             _serverList = _serverList.concat(info[y].miniIncrementServiceInfoVOs);
    //                         }
    //                     }
    //                     com[j]._serverList = _serverList;
    //                 }
    ...
    function isEmptyCart(data) {
        if (data.data.miniCartProfileVO == null || data.data.miniCommerceItemsGroupVOs.length === 0) {
            return true;
        }
    };
    ...
    function hoverMinCart() {
        ...
        $('.cartlink, .ucCart')
            ...
            .attr('href', '//cart' + cookieDomain + _site);
    }

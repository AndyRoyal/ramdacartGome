!function (exports, u, ui, uw, req, panel, tpl, validate, installRemind) {
    ...
    function transData(data) {
        function trans(data) {  
            ...
            r.no =  data.owerId || data.createTime  || data.modifyTime || data.mobileNumber || data.address.detailedAddress;
            ...
            var _detial = '';
            if (data.address.detailedAddress && data.address.detailedAddress.length > 12) {
                _detial = data.address.detailedAddress.substring(0, 12) + '…'
            } else {
                _detial = data.address.detailedAddress;
            }
            r.areas = [
    ...
    function init(div, data) {
         ...
         * @param list
         */
        function addressIsShow2(is, list) {
            var isSelected = false;

            /**
             * 设置全局 变量 addressNos
             */
            function setAddressNos(list) {
                addressNos = [];
                for (var i = 0, len = list.length; i < len; i++) {
                    addressNos.push(list[i].no );
                }
            }
            function sortByNo(list) {
                function _exchange(list, leftIndex, rightIndex) {
                    ...
                    list[rightIndex] = _val;
                }

                function _getListIndexByNo(list, no) {
                    for (var i = 0, len = list.length; i < len; i++) {
                        var _val = list[i];
                        if (_val.no && no && _val.no == no) {
                            return i;
                        }
                    }
                    return -1;
                ...
                for (var i = 0, len = list.length; i < len; i++) {
                    var _val = list[i], _no = addressNos[i];
                    if (_val.no !== _no) {
                        _exchange(list, i, _getListIndexByNo(list,_no));
                    }
                }
            }
            if (list.length) {
        ...
        // 给地址添加 【isShow】
        if (data.list) addressIsShow2($config.shoppingAddressAtom.more, data.list);
        ...
        //点击新增按钮
        div.find("[new-Address]").click(u.partial(newAddress, [div, { close: true, type: "new", data: data }]));
        //点击更多
        ...
        //设为默认地址
        div.find("[g-default-path]").on("click", function(){
            var div1 = $(this);
            var path = div1.attr("g-default-path").split(",");
            function selectfn(data) {
                var item = u.path(path, data);

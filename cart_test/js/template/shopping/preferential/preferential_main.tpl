...
<div class="yonghuzichan">
	...
	<div class="info-bd" g-pipe>
		...
		<!-- 优惠券 -->
		{{if $config.shoppingAtom.yhj == "Y" && $config.VBLE.youhuiquan()}}
                        ...
                        {{else}}
                            ...
                            {{if vrbsos.cvs.length > 0 && $config.shoppingAtom.yhj_jh=="N"}}
                                <dd class="use-combine">
                                    {{if $config.shoppingAtom.yhj_com=="N" && vrbsos.usedRGBSNum !== vrbsos.cvs.length}}
                                    <span class="" g-click="setOrState,useBestCom yhj_com Y,render"><i class="c-i checkboxs"></i>使用最优组合</span>
                                    {{else $config.shoppingAtom.yhj_com=="Y" && vrbsos.usedRGBSNum == vrbsos.cvs.length}}
                                    <span class="" g-click="setOrState,useBestComCanel yhj_com N,render"><i class="c-i checkbox_chose"></i>使用最优组合</span>
                                    {{else $config.shoppingAtom.yhj_com=="N" && vrbsos.usedRGBSNum == vrbsos.cvs.length}}
                                    <span class="" g-click="setOrState,useBestCom yhj_com Y,render"><i class="c-i checkboxs"></i>使用最优组合</span>
                                    {{else $config.shoppingAtom.yhj_com=="Y" && vrbsos.usedRGBSNum !== vrbsos.cvs.length}}
                                    <span class="" g-click="setOrState,useBestCom yhj_com Y,render"><i class="c-i checkboxs"></i>使用最优组合</span>
                                    {{/if}}
                                    &nbsp;&nbsp;&nbsp;<span class="discount-total">共抵用 <i class="fontRed fontArial">¥ ${$config.formatAmountWithNoRound(vrbsos.reduceRGBSAmount)}</i> 元{{if vrbsos.usedRGBSNum >= 0}}&nbsp;&nbsp;(使用<em class="fontArial">${vrbsos.usedRGBSNum}</em>张优惠券){{/if}}</span>
                                </dd>
                            {{/if}}
						{{/if}}
					</dl>
						...
						<ul class="coupon-group  clearfix">
							{{each(i, cvsitem) vrbsos.cvs}}
								...
								>
									...
									<div class="coupon normal"
                                         {{if cvsitem.checked}}
                                            g-click="changeCvsitem vrbsos.cvs.${i} N"
                                         {{else}}
                                            g-click="changeCvsitem vrbsos.cvs.${i} Y"
                                         {{/if}}
                                    >
                                    ...
                                    <div class='txt name_over2 useInfoBox'>
                                        ...
                                        {{if cvsitem.desc && cvsitem.desc != ""}}
                                            ...
                                            {{/if}}
                                        {{/if}}
                                        {{if cvsitem.couponApplyDetailInfoVO.skuList && cvsitem.couponApplyDetailInfoVO.skuList.length}}
							...
							{{each(i, cvsitem) vrbsos.ucvs}}
								<li class="coupon-dis easy">
                                    ...
                                    {{if cvsitem.couponApplyDetailInfoVO && cvsitem.couponApplyDetailInfoVO.unavailReasonType}}

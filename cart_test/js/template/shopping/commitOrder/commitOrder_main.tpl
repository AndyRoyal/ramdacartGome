<div class="clearfix commitOrderBody">	
	...
	{{if $config.VBLE.youhuiquan2() && $page.site != "warranty" && $page.site !== "qiyegou" && $page.site !== "secondHand"}}
	...
	<div class="text-right clearfix pr freight-container" id="freight-container">
		<span class="w185 fr pr20" >+¥${$config.formatAmount(haulage)}</span><span class="fr pr" hoverup>{{if data.freightProfileVO && data.freightProfileVO.freightForShopVOs && data.freightProfileVO.freightForShopVOs.length}}<em class="c-i tips_orange_icon pabs" style="left: -23px; top: 4px;"></em>{{/if}}运费：</span>
    ...
    {{/if}}
	<div class="text-right play clearfix" style="padding-right: 15px;">
	...
	{{if $config.VBLE.songhuorenxinxi()}}	
		...
		<div class="fr">
			...
			<p class="text-right mb10 pr20 pr10">
				<span class="mr10">收货人：${sname}</span>
				<span class="mr10">${phone}</span>
			</p>
		</div>
		</div>
	{{/if}}	
	<div  class="text-right play clearfix" style="background: #fff;">
